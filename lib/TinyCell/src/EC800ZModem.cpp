...
bool EC800ZModem::openSocket(const char *host, uint16_t port) {
  ...
  String resp;
  if (_at->waitResponse(150000, resp, "+QIOPEN: ") == 1) {
...
bool EC800ZModem::openSSLSocket(const char *host, uint16_t port) {
  ...
  // 1. Configure SSL context 0
  // Set security level: 0 = No verification, 1 = Server only, 2 = Server &
  // Client
  int secLevel = 0;
  if (_rootCA)
    secLevel = 1;
  ...
  _at->sendCommand("AT+QSSLCFG=\"seclevel\",0," + String(secLevel));
  _at->waitResponse(2000);

  // 2. Open SSL Socket
  // AT+QSSLOPEN=<pdpid>,<sslctxid>,<clientid>,"<host>",<port>,<access_mode>
  String cmd = "AT+QSSLOPEN=1,0,0,\"";
  cmd += host;
  cmd += "\",";
  cmd += port;
  cmd += ",0"; // 0 = Buffer mode
  ...
  // 3. Wait for asynchronous "+QSSLOPEN: " response
  String resp;
  if (_at->waitResponse(150000, resp, "+QSSLOPEN: ") == 1) {
    String line;
    _at->waitResponse(1000, line, "\r\n");
    int comma = line.indexOf(',');
    if (comma != -1) {
...
int EC800ZModem::getSignalQuality() {
  ...
  _at->waitResponse(1000, "OK\r\n"); // Ensure the command is fully finished
  return quality;
}
...
int EC800ZModem::send(const uint8_t *data, size_t len) {
  ...
  // Wait for the full line ending to prevent merging with subsequent Serial
  // logs
...
int EC800ZModem::available() {
  ...
  // Throttling: only poll the modem every 100ms to avoid flooding the AT
  // interface
  ...
  // Silence foreground debug for this background poll
  _at->setSilent(true);

  // Poll modem for actual bytes in buffer
...
int EC800ZModem::receive(uint8_t *buffer, size_t len) {
  ...
  // 1. If we have data in the internal prefetch buffer, serve it first
  if (_rxBufferLen > _rxBufferPos) {
  ...
  // 2. Buffer is empty, fetch more from modem
  _rxBufferPos = 0;
  _rxBufferLen = 0;

  // Prefetch up to RX_BUFFER_SIZE or the requested length, whichever is larger
  // but limited by modem's max read (usually 1500)
  ...
  // 3. Serve from the newly filled buffer
...
void EC800ZModem::processURC() { _at->processURC(); }
